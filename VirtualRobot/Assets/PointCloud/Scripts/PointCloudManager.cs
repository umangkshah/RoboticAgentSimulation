...
public class PointCloudManager : MonoBehaviour {
	...
	IEnumerator loadOFF(string dPath){
		...
		string npoints = sr.ReadLine (); // OFF
		//string[] buffer = sr.ReadLine ().Split(); // nPoints, nFaces
		
		numPoints = int.Parse (npoints);
		...
		for (int i = 0; i< numPoints; i++){
			...
			if (buffer.Length >= 5)
				colors[i] = new Color (int.Parse (buffer[5])/255.0f,int.Parse (buffer[4])/255.0f,int.Parse (buffer[3])/255.0f);
			else
				colors[i] = Color.cyan;
